...
@click.option(
    '--reset',
    is_flag=True,
    help='[PELIGRO] Eliminar y recrear todas las tablas'
)
@click.option(
    '--verify',
    is_flag=True,
    help='Solo verificar que las tablas existen (no crear)'
)
def init_db(db_path, reset, verify):
    ...
    try:
        ...
        try:
            ...
            click.secho(f'[FAIL] Tabla Activities no existe o tiene errores: {e}', fg='red', err=True)
        ...
        click.echo(f'  Tamaño:        {db_file.stat().st_size / 1024:.2f} KB' if db_file.exists() else '  Tamaño:        N/A')
        click.echo('='*60)
